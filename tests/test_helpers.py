...
def test_dft_heaviside():
    k = np.arange(-4, 5)
    Hk = dft_heaviside(k)
    print(Hk)
    Hks = []
    for i in k:
        Hks.append(dft_heaviside(i))
    Hks = np.asarray(Hks)
    print(np.linalg.norm(Hks - Hk))
    print(k)

def test_dft_aheaviside():
    nmesh = 10
    d = 20
    delt = 0.2
    k = np.arange(-nmesh,nmesh+1,1)
    Fk =  dft_aheaviside(d, delt, k, nmesh)
    print(Fk)
    

def test_aheaviside_ft():
    nmesh = 40
    d = 20
    delt = 0.2
    x = np.linspace(-np.pi, np.pi, nmesh+1, endpoint=True)
    F = aheaviside_ft(d, delt, x)
    print(F)
    plt.plot(x, F)
    plt.show()

def test_aheaviside_convol():
    nmesh = 40
    d = 20
    delt = 0.2
    x = np.linspace(-np.pi, np.pi, nmesh+1, endpoint=True)
    F = aheaviside_convol(d, delt, nmesh)
    plt.plot(x, F)
    plt.show()
...
def compare_aheaviside():
    ...
    delt = 0.2
    x = np.linspace(-np.pi, np.pi, nmesh+1, endpoint=True)
    F1 = aheaviside_convol(d, delt, nmesh)*(2*np.pi/(nmesh+1))
    F2 = aheaviside_ft(d, delt, x)
