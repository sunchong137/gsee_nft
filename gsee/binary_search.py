...
def bsearch_ground_state(
    ...
    (
        ...
        energy_acc, energy_rescalor, lower_bound_ovlp, fail_probability
    )
    ...
    if tot_num_samples > Ns_per_batch * num_batches:
        Ns_per_batch += 1
        print(
            "Adding {} samples to the ensamble.".format(
                Ns_per_batch * num_batches - tot_num_samples
            )
        )
    ...
    while x_L < x_R:
        ...
        for iter in range(num_batches):
            G_bar = acdf.acdf_kernel(
                max_dft_order, Fj, j_samp, Z_samp, energy_grid=x_M, nmesh=nmesh
            )
        ...
        if count_right <= num_batches / 2:
            x_R = x_M + 2 * rescaled_energy_acc / 3.0
        else:
            x_L = x_M - 2 * rescaled_energy_acc / 3.0
...
def gen_global_values(acc_energy, tau, lower_ovlp, fail_prob):
    ...
    d = int(
        1.0 / delt * np.log(1.0 / (delt * lower_ovlp))
    )  # cutoff of Fourier expansion
    batches = int(
        np.log(1.0 / fail_prob) + np.log(np.log(1.0 / delt))
    )  # number of batches to fulfill majority voting
    N_samp = int(1.0 / lower_ovlp**2 * (np.log(d) ** 2))  # number of total samples
